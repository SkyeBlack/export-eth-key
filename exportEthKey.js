...
app.post('/getprikey',function (req, res) {
	try{
		...
		var signstr = "address=" + address + ",currentDate=" + currentDate + ",password=" + password + ",key=" + key;
		var isSign = false;
...
app.post('/createAddress',function (req, res) {
	try{
		...
		var password = req.body.password;
		var sign = req.body.sign;
		var signstr = "currentDate=" + currentDate + ",password=" + password + ",key=" + key;
		var isSign = false;
		md5.string.quiet(signstr, function (err, md5) {
			if (err) {
				console.log(err);
